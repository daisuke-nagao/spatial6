...
//! A compact motion-subspace example using fixed Rust arrays.
//!
//! The same calculations appear in the `nalgebra` and `glam` examples.

use spatial6::{ArticulatedBodyInertia, ForceVector, MotionSubspace, MotionVector, SpatialInertia};
...
fn main() {
    ...
    // vector.
    let revolute_column = MotionVector::<f64>::new([0.0, 0.0, 1.0], [0.0; 3]);
    let revolute = MotionSubspace::<1>::from(revolute_column);
    ...
    let spherical = MotionSubspace::from_columns([
        MotionVector::new([1.0, 0.0, 0.0], [0.0; 3]),
        MotionVector::new([0.0, 1.0, 0.0], [0.0; 3]),
        MotionVector::new([0.0, 0.0, 1.0], [0.0; 3]),
    ]);
    ...
    let force = ForceVector::new([1.0, -2.0, 3.0], [0.7, -0.8, 0.9]);
    ...
    let rigid = SpatialInertia::<f64>::try_new(
        2.0,
        [0.5, -0.25, 0.75],
    ...
    let p_a = ForceVector::new([0.5, -1.5, 2.5], [0.2, -0.4, 0.6]);
    let tau = [2.0, -1.0, 3.0];
