...
//!
//! This is the same scenario as `examples/articulated_inertia_builtin.rs`, rebuilt
//! from native `nalgebra` types (`Nalgebra`) instead of plain arrays
//! (`Builtin`). `spatial6` only models rigid-body inertia
//! (`RigidBodyInertia`), so this example reimplements the articulated-body
//! inertia locally to demonstrate the concept end to end. See also
//! `examples/articulated_inertia_glam.rs` for the `glam` backend.

use nalgebra::{Matrix3, Rotation3, Vector3};
use spatial6::{
    ForceVector, InertiaError, MotionVector, Nalgebra, SpatialInertia, SpatialRepresentation,
    SpatialTransform,
};
...
/// stored as its twenty-one independent scalar parameters. Backend-agnostic
/// by construction: it only ever holds a plain `[f64; 21]`, converting to and
/// from the caller's native `Nalgebra::Matrix6` at each operation's boundary.
...
impl ArticulatedInertia {
    ...
    fn apply(&self, motion: &MotionVector<f64, Nalgebra>) -> ForceVector<f64, Nalgebra> {
        ForceVector::from_vector(Nalgebra::matrix6_vector_mul(
            &Nalgebra::matrix6_from_array(self.matrix()),
            &motion.to_vector(),
        ))
    }

    fn solve(&self, force: &ForceVector<f64, Nalgebra>) -> Option<MotionVector<f64, Nalgebra>> {
        Nalgebra::matrix6_solve_positive_definite(
            &Nalgebra::matrix6_from_array(self.matrix()),
            &force.to_vector(),
        )
    ...
    fn transformed(&self, transform: &SpatialTransform<f64, Nalgebra>) -> Self {
        let force_matrix = transform.force_matrix();
        let inverse_motion = transform.inverse().motion_matrix();
        let transformed = Nalgebra::matrix6_mul(
            &Nalgebra::matrix6_mul(&force_matrix, &Nalgebra::matrix6_from_array(self.matrix())),
            &inverse_motion,
        );
        Self::from_matrix_trusted(Nalgebra::matrix6_to_array(&transformed))
    }

    fn time_derivative(&self, velocity: &MotionVector<f64, Nalgebra>) -> Self {
        let cross_dual = velocity.cross_dual_matrix();
        let cross_motion = velocity.cross_matrix();
        let inertia = Nalgebra::matrix6_from_array(self.matrix());
        let dual_product = Nalgebra::matrix6_mul(&cross_dual, &inertia);
        let product = Nalgebra::matrix6_mul(&inertia, &cross_motion);
        Self::from_matrix_trusted(Nalgebra::matrix6_to_array(&Nalgebra::matrix6_sub(
            &dual_product,
            &product,
        )))
    }
...
fn main() {
    ...
    // own frame origin.
    let inertia = SpatialInertia::<f64, Nalgebra>::try_new(
        3.0,
        Vector3::new(0.2, 0.15, -0.1),
        Matrix3::new(0.4, 0.0, 0.0, 0.0, 0.6, 0.0, 0.0, 0.0, 0.5),
    )
    .unwrap();
    ...
    let full =
        ArticulatedInertia::try_from_matrix(Nalgebra::matrix6_to_array(&inertia.matrix())).unwrap();
    let motion = MotionVector::<f64, Nalgebra>::new(
        Vector3::new(0.1, -0.2, 0.3),
        Vector3::new(0.4, 0.0, -0.1),
    );
    let force = full.apply(&motion);
    assert!(max_abs_vector(force.to_array(), inertia.apply(&motion).to_array()) < 1.0e-9);
    let recovered_motion = full.solve(&force).unwrap();
    assert!(max_abs_vector(recovered_motion.to_array(), motion.to_array()) < 1.0e-9);
    ...
    assert!(
        max_abs_difference(
            ...
            Nalgebra::matrix6_to_array(&inertia.time_derivative(&motion))
        ) < 1.0e-9
    );
    ...
    let mut projected = Nalgebra::matrix6_to_array(&inertia.matrix());
    for row in 0..6 {
        for column in 0..6 {
    ...
    assert!(
        max_abs_vector(
            response.to_array(),
            ForceVector::<f64, Nalgebra>::zeros().to_array()
        ) < 1.0e-9
    );
    println!(
    ...
    let expected = Nalgebra::matrix6_to_array(&Nalgebra::matrix6_mul(
        &Nalgebra::matrix6_mul(
            &transform.force_matrix(),
            &Nalgebra::matrix6_from_array(locked.matrix()),
        ),
    ...
    assert!(
        max_abs_difference(
            ...
            Nalgebra::matrix6_to_array(&inertia.transformed(&transform).matrix())
        ) < 1.0e-9
    );
    ...
    let transposed = Nalgebra::matrix6_to_array(&Nalgebra::matrix6_transpose(
        &Nalgebra::matrix6_from_array(derivative),
    ));
